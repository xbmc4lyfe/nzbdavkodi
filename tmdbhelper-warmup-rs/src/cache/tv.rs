...
pub fn write_tv(conn: &mut Connection, t: &TvResponse) -> Result<()> {
    ...
    for g in &t.genres {
        tx.execute("INSERT OR IGNORE INTO genre (name, tmdb_id, parent_id) VALUES (?1, ?2, ?3)", params![&g.name, g.id, &item_id])?;
    }
    for lang in &t.spoken_languages {
        ...
        tx.execute("INSERT OR IGNORE INTO language (iso_language, parent_id) VALUES (?1, ?2)", params![&lang.iso_639_1, &item_id])?;
    }
    for c in &t.production_countries {
        dimensions::upsert_country(&tx, &c.iso_3166_1, &c.name)?;
        tx.execute("INSERT OR IGNORE INTO country (iso_country, parent_id) VALUES (?1, ?2)", params![&c.iso_3166_1, &item_id])?;
    }
    for co in &t.production_companies {
        ...
        tx.execute("INSERT OR IGNORE INTO studio (tmdb_id, parent_id) VALUES (?1, ?2)", params![co.id, &item_id])?;
    }
    for n in &t.networks {
        dimensions::upsert_broadcaster(&tx, n.id, &n.name, n.logo_path.as_deref(), n.origin_country.as_deref())?;
        tx.execute("INSERT OR IGNORE INTO network (tmdb_id, parent_id) VALUES (?1, ?2)", params![n.id, &item_id])?;
    }
    ...
    if let Some(ext) = &t.external_ids {
        ...
        if let Some(tvdb) = ext.tvdb_id {
            tx.execute("INSERT OR IGNORE INTO unique_id (key, value, parent_id) VALUES ('tvdb', ?1, ?2)", params![tvdb.to_string(), &item_id])?;
        }
    }
    tx.execute("INSERT OR IGNORE INTO unique_id (key, value, parent_id) VALUES ('tmdb', ?1, ?2)", params![t.id.to_string(), &item_id])?;

    // season stubs (full season details fetched separately by worker)
    for s in &t.seasons {
        write_season_stub(&tx, t.id, s, expiry)?;
    }
...
fn write_season_stub(tx: &rusqlite::Transaction, tv_id: i64, s: &SeasonStub, expiry: i64) -> Result<()> {
    ...
    if let Some(poster) = &s.poster_path {
        ...
        art::write_image(tx, &season_id, "posters", &img)?;
    }
    Ok(())
}
