...
def main():
    parser = argparse.ArgumentParser(
        description="Parse filename or torrent name using Parsett"
    )
    subparsers = parser.add_subparsers(dest="command", help="Commands")

    # Parse command
    parse_parser = subparsers.add_parser(
        "parse", help="Parse a filename or torrent name"
    )
    ...
    combine_parser.add_argument(
        "directory", type=str, help="Directory containing txt files"
    )
    ...
    dedupe_parser.add_argument(
        "filename", type=str, help="File to deduplicate and sort"
    )
    ...
    if args.command == "parse":
        ...
        result = parse_title(
            args.filename, translate_languages=args.translate_languages
        )
...
def combine_keywords(directory: str) -> None:
    ...
    print(
        f"Combined and sorted into {output_file.split('/')[-1]} using {files_used} files"
    )
