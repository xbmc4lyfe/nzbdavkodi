...
- **DV profile 7 guard.** Before committing to the fmp4 branch, `prepare_stream` runs `_probe_dv_profile` (a short `ffmpeg -i ... -f null -` whose stderr is scanned for `DOVI configuration record: ... profile: N`). If the source is confirmed profile 7 (dual-layer FEL with BL+EL+RPU), the session falls back to the matroska branch: fmp4 HLS cannot carry two HEVC layers in one track, so a P7 copy silently drops the enhancement layer and the Amlogic decoder is known to stall on the result. Profiles 5 and 8 and unknown/None are allowed through to fmp4.
